...
#........................stanford40_feature_fusion_baseline_classifier.py...................
...
MOMENTUM =0.09
BATCH_SIZE = 8
path_cross_features = '../data-stanford40/data13/'
...
filename = path_cross_features + 'stanford40_' + dim_str + 'aec_features_all_classes_tr_' + str(NUMBER_OF_CLASSES)
...
filename = path_cross_features + 'stanford40_' + dim_str + 'aec_features_all_classes_vl_' + str(NUMBER_OF_CLASSES)
tmp = scipy.io.loadmat(filename)
data_aec_valid = tmp['cross_feautures_val']
print 'NUmber of valid samples %d'%data_aec_valid.shape[0]

filename = path_cross_features + 'stanford40_' + dim_str + 'aec_features_all_classes_ts_' + str(NUMBER_OF_CLASSES)
tmp = scipy.io.loadmat(filename)
...
filename = path_cross_features_labels + 'stanford40_' + dim_str + 'feat_fusion_clsfr_train_labels'
tmp = scipy.io.loadmat(filename)
train_labels_all = tmp['train_labels']
train_labels_all = train_labels_all - 1

filename = path_cross_features_labels + 'stanford40_' + dim_str + 'feat_fusion_clsfr_valid_labels'
tmp = scipy.io.loadmat(filename)
...
#pdb.set_trace()	
filename = path_cross_features_labels + 'stanford40_' + dim_str + 'feat_fusion_clsfr_test_labels'
tmp = scipy.io.loadmat(filename)
...
for cls in range(NUMBER_OF_CLASSES):
	filename = path_cross_features + 'stanford40_' + dim_str + 'cec_features_class_tr_' + str(cls + 1) 
	print "Loading %s"%filename
	tmp = scipy.io.loadmat(filename)
	data_cec_train = tmp['cross_feautures_tr']
	
	filename = path_cross_features + 'stanford40_' + dim_str + 'cec_features_class_vl_' + str(cls + 1) 
	print "Loading %s"%filename
	tmp = scipy.io.loadmat(filename)
	...
	filename = path_cross_features + 'stanford40_' + dim_str + 'cec_features_class_ts_' + str(cls + 1) 
	print "Loading %s"%filename
	tmp = scipy.io.loadmat(filename)
	...
	classfet_train1 = aef_train1#np.hstack((aef_train1, cef_train1))
	#classfet_valid_all = aef_valid_all#np.hstack((aef_valid_all, cef_valid_all))
	classfet_test_all = aef_test_all#np.hstack((aef_test_all, cef_test_all))

	if SETWISE_TRAIN:
		classfet_train2 = aef_train2
		classfet_train3 = aef_train3
		classfet_train4 = aef_train4
		classfet_train5 = aef_train5

	classfet_test1 = aef_test1
	if SETWISE_TRAIN:	
		classfet_test2 = aef_test2
		classfet_test3 = aef_test3
		classfet_test4 = aef_test4
		classfet_test5 = aef_test5
		classfet_test6 = aef_test6
	#pdb.set_trace()	
...
model_feature_fusion.compile(optimizer=sgd,
      ...
      metrics=['accuracy'])
print model_feature_fusion.summary()

#Training
for itr in range(tr_range):
	end = 0	
	tr_list = []
	if itr == 0:
		print "*************************"
	...
	model_feature_fusion.fit(train_data_array, tr_labels, epochs=EPOCHS, batch_size=BATCH_SIZE, shuffle=True)
	print 'Training data (%d, %d)'%(tr_labels.shape[0], tr_data.shape[1])
	
for itr in range(ts_range):
	end = 0	
	tr_list = []
	ts_list = []
	if itr == 0:
	...
	filename = path_cross_features + 'for_baseline_confusion_mat'
	scipy.io.savemat(filename, dict(class_prob = prob, gt_test_labels = ts_labels))	
	print "**********"
