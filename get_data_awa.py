...
early_stopping = EarlyStopping(monitor='val_loss', min_delta=0.05, patience=10, verbose=1, mode='auto')
...
def function_get_input_data(obj_input_data):
	...
	if obj_input_data.dataset_name == 'AwA':
		...
		class_names = ['antelope', 'grizzly+bear', 'killer+whale', 'beaver', 
				...
				'humpback+whale', 'elephant', 'gorilla', 'ox',
				'fox', 'sheep', 'seal', 'chimpanzee', 'hamster',
				'squirrel', 'rhinoceros', 'rabbit', 'bat', 
...
def baseline_model_regression(visual_dim, semantic_dim):
	DIM1 = int(visual_dim * 0.8)
	DIM2 = int(visual_dim * 0.6)
	DIM3 = int(visual_dim * 0.5)
	model = Sequential()
...
def train_visual_to_semantic_nonspecific_regressor(obj):
	...
	if USE_SVR:
		...
		train_data_semantic = model.predict(obj.train_data)		
		valid_data_semantic = model.predict(obj.valid_data)		
		test_data_semantic = model.predict(obj.test_data)		
...
def get_data_for_seen_to_unseen_regressor(obj, seen_class, unseen_class):
	...
	ip_train = obj.train_attributes_cross[train_indices_seen_class_samples, :]
	ip_valid = obj.valid_attributes_cross[valid_indices_seen_class_samples, :]
...
def train_seen_to_unseen_regressor(obj, seen_class, unseen_class):
	# fix random seed for reproducibility
	seed = 7
	np.random.seed(seed)
	regressor_input, regressor_output, valid_input, valid_output = get_data_for_seen_to_unseen_regressor(obj, seen_class, unseen_class)
	...
	train_attributes_cross_mapped = model.predict(obj.train_attributes_cross)		
	valid_attributes_cross_mapped = model.predict(obj.valid_attributes_cross)		
	test_attributes_cross_mapped = model.predict(obj.test_attributes_cross)		
...
def get_data_for_visual_to_semantic_regressor(features_train, features_valid, obj, visual_class, semantic_class):
	...
	train_indices_output_class_samples = np.flatnonzero(obj.train_labels == semantic_class)
	valid_indices_input_class_samples = np.flatnonzero(obj.valid_labels == visual_class)
	valid_indices_output_class_samples = np.flatnonzero(obj.valid_labels == semantic_class)
	
	train_indices_input_class_samples = train_indices_input_class_samples[:MIN_SAMPLES_FOR_TRAIN,]
	train_indices_output_class_samples = train_indices_output_class_samples[:MIN_SAMPLES_FOR_TRAIN,]
	valid_indices_input_class_samples = valid_indices_input_class_samples[:MIN_SAMPLES_FOR_VALID,]
	valid_indices_output_class_samples = valid_indices_output_class_samples[:MIN_SAMPLES_FOR_VALID,]

	train_indices_input_class_samples = np.tile(train_indices_input_class_samples, train_indices_input_class_samples.shape[0])
	train_indices_output_class_samples = np.repeat(train_indices_output_class_samples, train_indices_output_class_samples.shape[0])
	valid_indices_input_class_samples = np.tile(valid_indices_input_class_samples, valid_indices_input_class_samples.shape[0])
	valid_indices_output_class_samples = np.repeat(valid_indices_output_class_samples, valid_indices_output_class_samples.shape[0])

	ip_train = features_train[train_indices_input_class_samples, :]
	ip_valid = features_valid[valid_indices_input_class_samples, :]
	op_train = obj.train_attributes_nsp[train_indices_output_class_samples, :]
	op_valid = obj.valid_attributes_nsp[valid_indices_output_class_samples, :]
	#NOTE: test_attributes_nsp CAN NOT be learned/obtained during training.
	...
	regressor_input = regressor_input + NOISE_FACTOR * np.random.normal(0, 1, regressor_input.shape)
	regressor_input = function_normalise_data(regressor_input)
	regressor_output = np.tile(op_train, (INCREASE_FACTOR, 1))
	regressor_output = regressor_output + NOISE_FACTOR * np.random.normal(0, 1, regressor_output.shape)
	regressor_output = function_normalise_data(regressor_output)
...
'''
-----------------------------------------------------------------
                        Regression f: visual --> semantic
-----------------------------------------------------------------
'''

def train_visual_to_semantic_specific_regressor(features_train, features_valid, features_test, obj, visual_class, semantic_class):
	# fix random seed for reproducibility
	seed = 7
	np.random.seed(seed)
...
def combine_specific_nonspecific_attributes(obj, cluster_labels, cluster_labels_cross, base_filename):
	...
	for classI in cluster_labels:
		...
		classI_indices_test = np.flatnonzero(obj.test_labels == classI)			
		norm_factor = np.sum(np.exp(weights_mat[classI_weights]))
		weights = weights_mat[k]/norm_factor
		...
		obj.test_attributes_cross[classI_indices_test, :] = ts
		k = 0
...
def visualize_data(data, labels, classes, dataset_class_names, string_title, figure_title, data_save_path):
	...
	for classI, color, name in zip(classes, colors, class_names):
		...
		plt.scatter(Y[indices, 0], Y[indices, 1], label = str(classI) + ':' + name, s = marker_size, c=color)
