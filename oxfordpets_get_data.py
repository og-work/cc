...
def function_get_input_data(obj_input_data):
	...
	print(BASE_PATH)
	if obj_input_data.dataset_name == 'oxfordpets':
		path_CNN_features_labels = BASE_PATH + "study/phd-research/data/datasets/oxford-pets/oxfordpets_dataset_features1.mat"
		tmp = scipy.io.loadmat(path_CNN_features_labels)
		...
		train_class_labels = np.arange(1, 38, 1)
	...
	obj_input_data.attributes = np.array([])#attributes 
	obj_input_data.visual_features_dataset = visual_features_dataset
	obj_input_data.dataset_labels = np.array([])#dataset_labels
	obj_input_data.dataset_train_labels = dataset_train_labels
	obj_input_data.dataset_test_labels = dataset_test_labels
	obj_input_data.train_sample_indices = np.array([])#train_sample_indices - 1
	obj_input_data.test_sample_indices = np.array([])#test_sample_indices - 1
