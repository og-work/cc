...
def function_init_weights(obj_train_tf_cc_input, this_weight):
	...
	if classI != classJ:
		base_filename = 'data3/' + dataset_name + '_' + str(input_dim) + '_'  + str(n_hidden) + '_'
		if this_weight == 'Wh':
...
def function_train_tensorflow_cc(obj_train_tf_cc_input):
	...
	base_filename = 'data3/' + dataset_name + '_' + str(input_dim) + '_' + str(n_hidden) + '_cl1_' +  str(classI) + '_cl2_' + str(classJ) + '_'
   ...
   #Wo = tf.transpose(Wh) # tied weights
		...
		for i in range(n_rounds):
			...
			for batch in range(number_of_batches):
				...
				if batch % 1000 == 0:
					...
					print "Epoch %6d Batch %6d:  train MSE %f valid MSE %f" %(i, batch, sess.run(meansq, feed_dict={x: obj_train_tf_cc_input.cc1_input_train_perm, y_:obj_train_tf_cc_input.cc1_output_train_perm}), \
	...
	#end tf session
	base_filename = 'data3/' + '_debug_wtinit_1_feat_fusion_clsfr' + str(obj_train_tf_cc_input.dim_feature) + '_' + str(dimension_hidden_layer1) + \
