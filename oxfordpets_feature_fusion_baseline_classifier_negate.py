...
#...........................oxfordpets_feature_fusion_baseline_classifier_negate.py................................
...
dim_feature = 50
DIM_INPUT = (NUMBER_OF_CLASSES - 1) * dim_feature#20 for sample data
...
MOMENTUM =0.09
BATCH_SIZE = 16
path_cross_features = '/home/SharedData/omkar/study/phd-research/codes/tf-codes/data-oxfordpets/data2_hemachandra/'
...
filename = path_cross_features + 'oxfordpets_' + dim_str + 'aec_features_all_classes_tr_' + str(NUMBER_OF_CLASSES)
...
filename = path_cross_features + 'oxfordpets_' + dim_str + 'aec_features_all_classes_vl_' + str(NUMBER_OF_CLASSES)
tmp = scipy.io.loadmat(filename)
data_aec_valid = tmp['cross_feautures_val']
print 'NUmber of valid samples %d'%data_aec_valid.shape[0]

filename = path_cross_features + 'oxfordpets_' + dim_str + 'aec_features_all_classes_ts_' + str(NUMBER_OF_CLASSES)
tmp = scipy.io.loadmat(filename)
...
filename = path_cross_features_labels + 'oxfordpets_' + dim_str + 'feat_fusion_clsfr_train_labels'
tmp = scipy.io.loadmat(filename)
train_labels_all = tmp['train_labels']
train_labels_all = train_labels_all - 1

filename = path_cross_features_labels + 'oxfordpets_' + dim_str + 'feat_fusion_clsfr_valid_labels'
tmp = scipy.io.loadmat(filename)
...
filename = path_cross_features_labels + 'oxfordpets_' + dim_str + 'feat_fusion_clsfr_test_labels'
tmp = scipy.io.loadmat(filename)
...
for cls in range(NUMBER_OF_CLASSES):
	filename = path_cross_features + 'oxfordpets_' + dim_str + 'cec_features_class_tr_' + str(cls + 1) 
	print "Loading %s"%filename
	tmp = scipy.io.loadmat(filename)
	data_cec_train = tmp['cross_feautures_tr']
	
	filename = path_cross_features + 'oxfordpets_' + dim_str + 'cec_features_class_vl_' + str(cls + 1) 
	print "Loading %s"%filename
	tmp = scipy.io.loadmat(filename)
	...
	filename = path_cross_features + 'oxfordpets_' + dim_str + 'cec_features_class_ts_' + str(cls + 1) 
	print "Loading %s"%filename
	tmp = scipy.io.loadmat(filename)
	...
	classfet_train1 = cef_train1
	#classfet_test_all = np.hstack((aef_test_all, cef_test_all))
	if SETWISE_TRAIN:
		classfet_train2 = cef_train2
		classfet_train3 = cef_train3
		classfet_train4 = cef_train4
		classfet_train5 = cef_train5

	classfet_test1 = cef_test1
	if SETWISE_TRAIN:	
		classfet_test2 = cef_test2
		classfet_test3 = cef_test3
		classfet_test4 = cef_test4
		classfet_test5 = cef_test5
		classfet_test6 = cef_test6
...
model_feature_fusion.compile(optimizer=OPTIMIZER,
...
for itr in range(tr_range):
	...
	for k in range(NUMBER_OF_CLASSES):
		start = end
		end = start + (NUMBER_OF_CLASSES - 1)*dim_feature
...
for itr in range(ts_range):
	...
	filename = path_cross_features + 'for_confusion_mat'
